...
import { FaStar, FaRegStar, FaChevronDown, FaChevronUp, FaTrash, FaPlus } from 'react-icons/fa';

const updateTodoInArray = (todos, updatedTodo) =>
  todos.map(todo => {
    if (todo.id === updatedTodo.id) return updatedTodo;
    if (todo.subTasks && todo.subTasks.length > 0) {
      return { ...todo, subTasks: updateTodoInArray(todo.subTasks, updatedTodo) };
    }
    return todo;
  });
...
const removeTodoFromArray = (todos, idToRemove) =>
  todos
    ...
    .map(todo => ({
      ...todo,
      subTasks: todo.subTasks ? removeTodoFromArray(todo.subTasks, idToRemove) : [],
    }));

function TodoItem({ todo, setTodos, parentTodo = null }) {
  ...
  const subTaskButtonLabel = (!todo.subTasks || todo.subTasks.length === 0) 
    ? 'Add Subtask' 
    : showSubTasks ? 'Hide Subtasks' : 'Show Subtasks';

  return (
    <div className={`todo-item ${todo.priority ? 'priority' : ''} ${parentTodo ? 'sub-task' : ''}`}>
      <div className="todo-content">
        ...
        />
        <span className={todo.taskStatus ? 'completed' : ''}>{todo.taskName}</span>

        {!parentTodo && (
          <div className="button-group">
            <button onClick={togglePriority} className="icon-button" aria-label={todo.priority ? 'Unmark Priority' : 'Mark as Priority'}>
              {todo.priority ? <FaStar /> : <FaRegStar />}
            </button>
            <button onClick={() => setShowSubTasks(prev => !prev)} className="icon-button" aria-label={showSubTasks ? 'Hide Subtasks' : 'Show Subtasks'}>
              {showSubTasks ? <FaChevronUp /> : <FaChevronDown />}
            </button>
            <button onClick={handleDelete} className="icon-button delete-button" aria-label="Delete Task">
              <FaTrash />
            </button>
          </div>
        )}
